...
秘密鍵を知る攻撃者による除去、生成AIによる再生成、幾何補正を伴う高度な同期攻撃、暗号学的な真正性証明は対象外です。この実装は研究・学習用であり、DRMや著作権侵害の自動判定には使用できません。
...
合成画像3枚、識別子`creator-123`、DCT強度45・反復5で、120組み合わせを実行した初期結果です。小規模な合成データによるパイプライン確認であり、方式一般の性能を示す結論ではありません。
...
加工なしのDCT埋め込み画像は平均PSNR 36.70 dB、平均SSIM 0.912でした。全測定値と画像単位の失敗は`results/default/raw_results.csv`、集計値は`summary.csv`で確認できます。
...
- 合成サンプル3枚は動作確認用で、統計的な結論を出すには不足している
